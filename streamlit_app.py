...
SAMPLE_EMPTY = {k: 0.0 for k in SAMPLE_LEGIT}
...
if 'current_data' not in st.session_state:
    st.session_state['current_data'] = SAMPLE_EMPTY.copy()
...
def get_model_info():
    try:
        response = requests.get(
            f"{API_URL}/model-info", timeout=30
        )
        return response.json()
    except:
        return None
...
def create_shap_chart(explanation):
    fraud_signals = explanation.get('top_fraud_signals', [])
    legit_signals = explanation.get('top_legit_signals', [])
...
with st.sidebar:
    ...
    if health:
        ...
        st.metric("AUC-ROC", health.get('auc_roc', 'N/A'))
        st.metric("Threshold", health.get('threshold', 0.5))
    else:
    ...
    if model_info:
        st.metric("Model", model_info.get('model_type', 'XGBoost'))
        st.metric("Precision", model_info.get('fraud_precision', 'N/A'))
        st.metric("Recall", model_info.get('fraud_recall', 'N/A'))
        st.metric("F1 Score", model_info.get('fraud_f1', 'N/A'))
...
with tab1:
    ...
    with col_btn1:
        if st.button("📋 Load Sample Legit",
            ...
            st.session_state['current_data'] = SAMPLE_LEGIT.copy()
            st.session_state['prediction'] = None
            st.rerun()
    ...
    with col_btn2:
        if st.button("🚨 Load Sample Fraud",
                     use_container_width=True):
            st.session_state['current_data'] = SAMPLE_FRAUD.copy()
            st.session_state['prediction'] = None
            st.rerun()
    ...
    # ── READ CURRENT DATA FROM SESSION STATE ──
    ...
    with col1:
        ...
        time_val = st.number_input(
            "Time (seconds)",
            value=float(data.get('Time', 0.0)),
            min_value=0.0,
            help="Seconds elapsed since first transaction"
        )
        amount_val = st.number_input(
            "Amount (€)",
            value=float(data.get('Amount', 0.0)),
            min_value=0.0,
    ...
    for i in range(1, 29):
        ...
        with cols[col_idx]:
            v_values[f'V{i}'] = st.number_input(
                f"V{i}",
                value=float(data.get(f'V{i}', 0.0)),
                format="%.4f"
            )
    ...
    if prediction:
        if "error" in prediction:
            ...
            with res_col1:
                ...
                with m3:
                    st.metric("Threshold", prediction['threshold_used'])
            ...
            with res_col2:
                if 'explanation' in prediction:
                    ...
                    shap_chart = create_shap_chart(explanation)
                    st.plotly_chart(shap_chart, use_container_width=True)

                    st.markdown("**🚨 Fraud Signals:**")
                    for sig in explanation['top_fraud_signals']:
                        st.error(
                            f"**{sig['feature']}** = "
                            f"{sig['value']:.4f} → "
                    ...
                    for sig in explanation['top_legit_signals']:
                        st.success(
                            f"**{sig['feature']}** = "
                            f"{sig['value']:.4f} → "
                            f"impact: {sig['impact']:.4f}"
...
with tab2:
    ...
    if model_info:
        ...
        with c2:
            st.metric("Precision",
                      model_info.get('fraud_precision', 'N/A'),
        ...
        with col1:
            ...
            details = {
                ...
                "Total Features": model_info.get('total_features', 31),
        ...
        with col2:
            ...
            metrics_data = {
                'Metric': ['AUC-ROC', 'Precision', 'Recall', 'F1'],
                'Score': [
                    model_info.get('auc_roc', 0),
                    model_info.get('fraud_precision', 0),
